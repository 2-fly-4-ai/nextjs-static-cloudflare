...
const Page = ({ data }) => {
	...
	product_list = product_list.filter((v, i, a) => a.findIndex(t => (JSON.stringify(t) === JSON.stringify(v))) === i)
				...
				<div className="py-8 px-4 mx-auto max-w-screen-2xl text-center lg:py-0 lg:px-5 flex ">
					...
					<div className="sm:hidden lg:flex w-80 mb-5 flex-col border-r">
						...
						{!isEmpty(data?.page?.nodes[0]?.children?.nodes) ? (
							<div className="space-x-2 mt-2 flex flex-col">
								<span className="text-gray-800 mx-2 my-4 text-left font-semibold">Sub-Categories: </span>
								<div>
								...
								<ul className='flex flex-wrap  align items-start gap-1 p-2'>
									{data?.productBrands?.nodes.map(brand => (
										brand?.seo?.metaRobotsNoindex != "noindex" ?
											...
											<li key={brand?.name} className="text-gray-500 dark:text-gray-400 mb-3">
												<Link href={brand.uri}>
													<a className="text-gray-600 border border-gray-500 rounded-full px-2 overflow-hidden text-sm py-0.5 pb-1.5 hover:bg-gray-200" dangerouslySetInnerHTML={{ __html: sanitize(brand?.name ?? {}) }} />
												</Link>
											</li> : null
					...
					<div className="flex-1 flex-col px-3">
						...
						<div className="flex flex-col items-start rounded-lg my-5">
							<h1><a className="m-5 text-gray-600 text-5xl px-2  py-2 pb-1.5" href={data?.page?.nodes[0]?.name}>{data?.page?.nodes[0]?.name}</a></h1>
							{/* <p className="text-gray-400 p-1 ml-1 text-lg font-medium">RESULTS</p> */}
							<p className="text-gray-600 font-medium m-5">

								{!isEmpty(data?.page?.nodes[0]?.parent?.node?.parent?.node?.parent?.node?.name) ? (
									<a className="mx-1 text-sm text-gray-600 border border-gray-500 rounded-full px-4  py-1 pb-1.5 hover:bg-gray-200" href={data?.page?.nodes[0]?.parent?.node?.parent?.node?.uri}>{data?.page?.nodes[0]?.parent?.node?.parent?.node?.parent?.node?.name}</a>
								) : null}

								{!isEmpty(data?.page?.nodes[0]?.parent?.node?.parent?.node?.name) ? (
									<a className="mx-1 text-sm text-gray-600 border border-gray-500 rounded-full px-4  py-1 pb-1.5 hover:bg-gray-200" href={data?.page?.nodes[0]?.parent?.node?.parent?.node?.uri}>{data?.page?.nodes[0]?.parent?.node?.parent?.node?.name}</a>
								) : null}

								{!isEmpty(data?.page?.nodes[0]?.parent?.node?.name) ? (
									<a className="mx-1 text-sm text-gray-600 border border-gray-500 rounded-full px-4  py-1 pb-1.5 hover:bg-gray-200" href={data?.page?.nodes[0]?.parent?.node?.uri}>{data?.page?.nodes[0]?.parent?.node?.name}</a>
								) : null}
								...
								{product_list.map((product, index) => {
									...
									return (
										<div className="p-0 pb-3 h-68 max-w-sm bg-white rounded-none  dark:bg-gray-800 dark:border-gray-700">
											...
											{/* modal */}
											<div id="readProductModal" tabindex="-1" aria-hidden="true" className={`${isMenuVisible && index == activeId ? 'flex' : 'hidden'} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center bg-black bg-opacity-60 w-screen h-screen`}>
														...
														<div className='flex overscroll-contain'>
															<div className="flex justify-between mb-4 rounded-t sm:mb-5">
																<div className="text-lg w-80 justify-center items-center flex text-gray-900 md:text-xl dark:text-white">
																	<Image src={product?.single_product_acf?.productImageMainUrl
																	} height="256" width="256" objectFit="cover" />
																</div>
															</div>
															<dl className='text-left p-6'>
																<div className='absolute right-3 top-3'>
																	<button onClick={() => { setMenuVisibility(!isMenuVisible); activeCategory(index) }} className={isActive(index) ? "..." : ""} type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="readProductModal">
											...
											<h5 className="py-3 text px-2 tracking-tight text-gray-900 font-semibold dark:text-white uppercase hover:text-blue-500"><a href={product?.uri}>{product?.title.split(" ").slice(0, 8).join(' ')}</a></h5>
